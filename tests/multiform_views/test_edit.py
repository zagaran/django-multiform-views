...
from multiform_views.edit import FormsMixin
from . import views
from .forms import JournalistForm, CommentForm
from .models import Journalist, Article


class FormsMixinTests(SimpleTestCase):
    request_factory = RequestFactory()

    def test_initial_data(self):
        """ Test instance independence of initial data dict (see #16138) """
        initial_1 = FormsMixin().get_initial('form_key')
        initial_1['foo'] = 'bar'
        initial_2 = FormsMixin().get_initial('form_key')
        self.assertNotEqual(initial_1, initial_2)

    def test_get_form(self):
        class TestFormsMixin(FormsMixin):
            request = self.request_factory.get('/')

        class GetFormMethodTestFormsMixin(TestFormsMixin):
            def get_form_1_class(self):
                return forms.Form

        self.assertIsInstance(
            GetFormMethodTestFormsMixin().get_form('form_1'), forms.Form,
            'get_form() should use custom form method if one is provided.'
        )

        class GetFormClassTestFormsMixin(TestFormsMixin):
            form_classes = {
                'form_1': forms.Form
            }

        self.assertIsInstance(
            GetFormClassTestFormsMixin().get_form('form_1'), forms.Form,
            'get_form() should use form_classes if not custom method is provided.'
        )

    def test_get_context_data(self):
        class FormsContext(FormsMixin):
            request = self.request_factory.get('/')
            form_classes = {
                'form_1': forms.Form
            }

        self.assertIsInstance(FormsContext().get_context_data()['form_1'], forms.Form)
...
class BasicFormTests(TestCase):
    ...
    def test_post_data(self):
        res = self.client.post('/journalist/form', {
            'name': "Me",
            'slug': "slug-slug-goose",
            'email': "email@email.com",
            '_key': "journalist_form"
        })
        self.assertRedirects(res, '/journalist/detail')
